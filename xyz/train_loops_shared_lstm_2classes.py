# train_loops_shared_lstm_2classes.py
# Shared LSTM per loop (3 loops per graph).
#
# Dataset:
#   root/npz/<class_label>/*.npz
#   each npz contains X: float32 of shape (3, L, 3)  (3 loops)
#
# Model idea:
#   For each loop (L,3) -> shared encoder (LSTM/GRU) -> embedding
#   3 embeddings -> concat -> MLP -> softmax
#
# Extras:
# - stratified split per class (train/val/test)
# - balanced subset: take min(cap, len(class1), len(class2)) from each class
# - optional permutation augmentation of loops (order invariance): --permute_loops
# - metrics: confusion matrix + precision/recall/f1 + predictions.csv
# - run_summary.json + file lists for reproducibility
#
# Recommended run (your case):
#   python ...\train_loops_shared_lstm_2classes.py ^
...
#     --expected_L 201 ^
#     --permute_loops ^
#     --out "C:\Users\danil\.vscode\grafy\results\sharedLSTM_3_1_vs_4_1"
...
def stratified_split_two_classes(
    ...
    Stratified per-class split:
      - sample effective_cap from each class
      - split each class into train/val/test with same fractions
      - concatenate corresponding splits and shuffle
    """
    rng = random.Random(seed)
...
def _np_load_loops_raw(
    ...
    # Optional loop permutation augmentation (order invariance)
    if permute_loops:
        # deterministic per file+seed if you want:
        # create a stable int from path
...
def make_tf_dataset(
    ...
    def _map_fn(p):
        X, y = tf.numpy_function(
            func=lambda pb: _np_load_loops_raw(pb, class_to_id, expected_L, permute_loops, seed),
            inp=[p],
...
def build_shared_loop_model(
    ...
    """
    Input: (3, L, 3)
    We apply the same encoder to each of the 3 loops, then concat embeddings.
    """

    inp = tf.keras.Input(shape=(3, cfg.expected_L, 3), name="X")  # (batch,3,L,3)

    # Split loops: X[:,0], X[:,1], X[:,2] => (batch,L,3)
    loops = [
    ...
    def rnn_layer(units: int, return_sequences: bool):
        if cfg.rnn_type == "GRU":
            layer = tf.keras.layers.GRU(units, return_sequences=return_sequences)
        else:
            layer = tf.keras.layers.LSTM(units, return_sequences=return_sequences)
        if cfg.bidirectional:
            return tf.keras.layers.Bidirectional(layer)
        return layer

    # Shared stack: we re-use the same layer instances for all loops
    shared_layers: List[tf.keras.layers.Layer] = []
    for li in range(cfg.rnn_layers):
        # return_sequences True for all but last layer in encoder
        rs = li < cfg.rnn_layers - 1
        shared_layers.append(rnn_layer(cfg.rnn_units, return_sequences=rs))
        if cfg.dropout > 0:
    ...
    def encode_loop(x):
        for layer in shared_layers:
            x = layer(x)
        return x  # (batch, units*2 if bidi else units)
...
def main():
    ...
    ap.add_argument("--rnn_layers", type=int, default=1, help="Encoder layers (shared)")
    ap.add_argument("--bidirectional", action="store_true", help="Use Bidirectional RNN in encoder")
    ap.add_argument("--dropout", type=float, default=0.2)
    ap.add_argument("--dense_units", type=int, default=64)

    ap.add_argument("--out", default=str(PROJECT_ROOT / "results" / "shared_lstm_2class"))
    ...
    print(f"Train config: epochs={args.epochs} bs={args.bs} lr={args.lr} seed={args.seed} permute_loops={args.permute_loops}")
    print(f"Model config: {args.rnn_type} units={args.rnn_units} layers={args.rnn_layers} bidi={args.bidirectional} "
          f"dropout={args.dropout} dense_units={args.dense_units}")

    train_ds = make_tf_dataset(
        train_files, class_to_id, args.expected_L, args.bs,
        shuffle=True, seed=args.seed, permute_loops=args.permute_loops
    )
    val_ds = make_tf_dataset(
        val_files, class_to_id, args.expected_L, args.bs,
        shuffle=False, seed=args.seed, permute_loops=False  # do not augment val
    )
    test_ds = make_tf_dataset(
        test_files, class_to_id, args.expected_L, args.bs,
        shuffle=False, seed=args.seed, permute_loops=False  # do not augment test
    )
    ...
    cfg = ModelConfig(
        ...
        emb_dim=int(args.rnn_units) * (2 if args.bidirectional else 1),
    ...
    model = build_shared_loop_model(cfg, n_classes=2, lr=args.lr)
    print("\nModel summary:")
    model.summary()
    ...
    summary = {
        ...
        "input": {"L": int(args.expected_L), "loops": 3, "coords": 3},
        "train_config": {
