# train_sts_2d.py
# Shared 2D CNN per loop for StS classification.
#
# Input per sample:
#   StS: (3, L, L)
# We treat each of the 3 loop matrices separately with the same 2D encoder.
#
# Recommended first run:
# python train_sts_2d.py ^
...
#   --out "C:\Users\danil\.vscode\grafy\experiments\sts_3_1_vs_4_1\run03_cnn2d_shared_cliplogz_s42" ^
#   --results_csv "C:\Users\danil\.vscode\grafy\experiments\sts_3_1_vs_4_1\results_2d.csv"
...
def preprocess_sts_per_sample(X: np.ndarray, mode: str, clip_lo: float, clip_hi: float, eps: float) -> np.ndarray:
    """
    Debug-only per-sample preprocessing.
    X: (3, L, L)
    """
...
def _np_load_sts_with_aug(
    ...
    # augmentation: only loop permutation, no reversal
...
def make_tf_dataset(
    ...
    def _map_fn(p):
        X, y = tf.numpy_function(
            func=lambda pb: _np_load_sts_with_aug(
                pb,
                class_to_id,
                expected_L,
                seed,
...
def build_shared_cnn2d_sts(cfg: Cnn2DCfg, n_classes: int, lr: float) -> tf.keras.Model:
    inp = tf.keras.Input(shape=(3, cfg.expected_L, cfg.expected_L, 1), name="StS")
    ...
    pool2d = tf.keras.layers.MaxPooling2D(pool_size=(2, 2), padding="same")
...
def main():
    ...
    ap.add_argument("--variant", default="StS", help="npz key, usually StS")

    ap.add_argument("--model", default="cnn2d_shared", choices=["baseline_lr", "cnn2d_shared"])
    ...
    if args.model == "baseline_lr":
        ...
        model = build_shared_cnn2d_sts(cfg, n_classes=2, lr=args.lr)
        model_cfg_blob = asdict(cfg)
